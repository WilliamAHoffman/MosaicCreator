...
public class main {
    ...
    static int[] averageColor(File texture){
        try {
            ...
            System.out.println(texture + " color failure");
            texture.delete();
            return null;
        }
    }
    ...
    static void createMosaic(ArrayList<File> textures, ArrayList<int[]> RGB, File picture){
        try {
            ...
            System.out.println(picture + " mosaic failure");
        }
    }
    ...
    static ArrayList<String> readFile(String name){
        try {
            ...
            System.out.println("palette error");
            return null;
        }
    }

    static ArrayList<File> textureStartUp(){
        File textureFolder = new File("block");
        ArrayList<File> textures = new ArrayList<>();
        ArrayList<String> Palette = readFile("palette.txt");
        ArrayList<String> antiPalette = readFile("anti-palette.txt");

        if(Palette.size() == 0){
            for(File texture : textureFolder.listFiles()){
                if(!antiPalette.contains(texture.getName())){
                    textures.add(new File(texture + ""));
                }
            }
        }
        else {
            for(String texture : Palette){
                textures.add(new File("block\\" + texture + ".png"));
            }
        }
        return textures;
    }
    ...
    static File resizeImage(File image, int size){
        try{
            ...
            System.out.println(image + " resize failure");
            return null;
        }
    }
